...
class PLMActor (
  ...
  var mongoService: MongoActor = new MongoActor()
    ...
    case msg: JsValue =>
      ...
      cmd.getOrElse(None) match {
        ...
        case "getExercise" =>
          ...
          (optDescription.getOrElse(None), optCode.getOrElse(None), optExerciseName.getOrElse(None)) match {
        	  case (description: String, code: String, exerciseName: String) =>
              mongoService.insert(description, code, exerciseName)
            case (_, _, _) =>
